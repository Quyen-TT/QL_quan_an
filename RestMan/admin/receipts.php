                        ...
                        <div class="table-responsive">
                            <table class="table align-items-center table-flush text-center">
                                ...
                                <tbody>
                                    ...
                                    if (!empty($search)) {
                                        $ret = "SELECT * FROM rpos_orders WHERE order_status = 'Đã thanh toán' AND (order_code LIKE ? OR created_at LIKE ? OR customer_name LIKE ?)";
                                        $stmt = $mysqli->prepare($ret);
                                        $searchParam = "%{$search}%";
                                        $stmt->bind_param('sss', $searchParam, $searchParam, $searchParam);
                                    } else {
                                        $ret = "SELECT * FROM rpos_orders WHERE order_status = 'Đã thanh toán' ORDER BY `rpos_orders`.`created_at` DESC";
                                        $stmt = $mysqli->prepare($ret);
                                    }
                                    $stmt->execute();
                                    $res = $stmt->get_result();
